...
describe("BasicStakeUnstake", function () {
  ...
  beforeEach(async function () {
    ...
    balRewardPool = await t5.deploy(depositToken.address, BALToken.address, stakingRewardsContract.address);
  ...
  describe("Test initial values after Deployment", function () {
    ...
    it("Should enableBalRewardPool have all parameters ", async function() {
      await expect(stakingRewardsContract.connect(operator)
      .enableBalRewardPool(depositToken.address, nullAddress, nullAddress))
      .to.be.revertedWith('!badParameter');

      await expect( stakingRewardsContract.connect(operator)
      .enableBalRewardPool(nullAddress, balRewardPool.address, nullAddress))
      .to.be.revertedWith('!badParameter');

      await expect( stakingRewardsContract.connect(operator)
      .enableBalRewardPool(nullAddress,nullAddress, balancerGauge.address))
      .to.be.revertedWith('!badParameter');
    });

    it("Should enableBalRewardPool call only once ", async function() {
      // enable BAL Reward Pool
      await stakingRewardsContract.connect(operator)
      .enableBalRewardPool(depositToken.address, balRewardPool.address, balancerGauge.address);
    ...
    it("Should enableBalRewardPool work as expected", async function () {
      // initial value of DEPOSIT_TOKEN_ADDR and BAL_REWARD_POOL and balancerGauge should be null address
    ...
    it("Should stake/claim/withdraw work as expected", async function () {
      ...
      // claim BAL rewards with an INVALID_BAL_REWARD_POOL
      await expect(stakingRewardsContract.connect(userA)
        .claimBALRewards())
        .to.be.revertedWith('INVALID_BAL_REWARD_POOL');

      // earn BAL rewards with an INVALID_BAL_REWARD_POOL
      await expect(stakingRewardsContract.earnedBALRewards(userA.address))
        .to.be.revertedWith('INVALID_BAL_REWARD_POOL');
   
      // enable Bal Reward Pool correctly
      await stakingRewardsContract.connect(operator)
        .enableBalRewardPool(
          depositToken.address,
          balRewardPool.address,
      ...
      // should earnedTokenRewards and earnedBALRewards be zero before stake
    ...
    it("Should stake/withdraw with/without Bal Reward Pool", async function () {
      ...
      // mint token and approve the contract in order to give permission
      await stakeToken.connect(userA).mint(stakeAmount * 2);
      await stakeToken.connect(userA).approve(stakingRewardsContract.address, stakeAmount * 2);
      ...
      //  should bal reward pool reverted with !BAL_REWARD_POOL
      await expect(stakingRewardsContract.connect(userA).transferStakedBalanceToBalRewardPool())
      ...
      await stakingRewardsContract.earnedBALRewards(userA.address);
    ...
    it("Should stake/withdraw with BAL Reward Pool from the beginning", async function () {
      ...
      // mint token and approve the contract in order to give permission
      await stakeToken.connect(userA).mint(stakeAmount);
      await stakeToken.connect(userA).approve(stakingRewardsContract.address, stakeAmount);
