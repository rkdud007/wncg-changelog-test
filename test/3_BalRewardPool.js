...
describe("BALRewardPool", function () {
  ...
  describe("Test initial values after Deployment", function () {
    ...
    it("Should stakeFor and withdrawFor work as expected", async function () {
      ...
      // cannot stake 0
      expect(balRewardPoolContract.connect(operator).stakeFor(userA.address, 0))
      .to.be.revertedWith('RewardPool : Cannot stake 0');
    ...
    it("Should rewardRate be > 0 when total supply is not zero and queueNewRewards was executed", async function () {
      ...
      // processIdleReward when reward > 0
      await balRewardPoolContract.processIdleRewards();
    ...
    it("get reward not reward is 0", async function(){
      const amount = 10000000;
      // mint
      await token1.connect(operator).mint(operator.address, amount);
      await token1.connect(operator).mint(balRewardPoolContract.address, amount);

      // approve
      await token1.connect(operator).approve(balRewardPoolContract.address, amount);

      // stake for user A (msg.sender: operator)
      await balRewardPoolContract.connect(operator).stakeFor(userA.address, amount);

      // queueNewRewards will update rewardRate
      await balRewardPoolContract.connect(operator).queueNewRewards(amount);

      // mint
      await token2.connect(operator).mint(balRewardPoolContract.address, amount * 2);

      // put evm time to future
      await network.provider.send("evm_increaseTime", [10]);
      await network.provider.send("evm_mine");

      // queueRatio < newRewardRatio
      await balRewardPoolContract.connect(operator).queueNewRewards(amount);

      // put evm time to future
      await network.provider.send("evm_increaseTime", [60 * 60 * 24 * 6])
      await network.provider.send("evm_mine")
      ...
      // reward > 0, getReward
      await balRewardPoolContract.getReward(userA.address);  
     });
  });
